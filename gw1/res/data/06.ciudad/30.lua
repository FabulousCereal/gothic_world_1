return {
	{"style", "vnMaria"},
	res.fun.macro.title,

	{"name", "María"},
	[[Nos quedamos en silencio tras la historia de Croft. Bake tomó de su
mate y Croft sorbeteó de su café. El viento agarraba fuerza una vez mas.]],

	{"name", "Falcon"},
	[["Es raro eso de la barricada. ¿Será para que no salgan los
infectados?"]],

	{"name", "Bake"},
	[["La nube de la explosión era gigante. Salió en la tele," dijo sin
levantar la vista. "No creo que solo acá hayan zombies."]],

	{"name", "María"},
	[["Pero a lo mejor afuera ya lo controlaron." Bake se pasó la mano por
el pelo y suspiró.]],

	{"name", "Bake"},
	[["¿Que querés que diga? Se están demorando su buen rato en entrar."]],

	{"name", "María"},
	[[Bake se sirvió otro mate. Se veía un poco como en la mañana cuando
entré a la cocina. Tenso, preocupado de algo.]],

	{"name", "Croft"},
	[["¿No han intentado salir de la ciudad?"]],

	{"name", "María"},
	[["Pues... No. Tenía miedo y me quedé en el hotel donde estaba por tres
días."]],

	{"name", "Bake"},
	[["Yo igual. Preferí quedarme acá porque en todos lados sería lo mismo.
Y en todo caso no tenía vehículo."]],

	{"name", "Falcon"},
	[["Yo pensé en hacerlo pero... ehh, creí que podría recordar algo si me
ponía a conducir por cualquier lado."]],

	{"name", "Croft"},
	[["¿Recordar?" preguntó Croft. Falcon trató de acomodarse en la silla.]],

	{"name", "Falcon"},
	[["Perdí la memoria. Estaba al lado de la fábrica cuando
explotó, y no recuerdo nada para atrás. Me dicen Falcon por la chapita. No sé
cual será mi nombre real."]],

	{"name", "María"},
	[[Croft asintió mientras escuchaba, pero prefirió no comentar nada al
respecto. Bake chupeteó su mate hasta el final, y Croft se acordó de terminar
su café.]],

	{"name", "Bake"},
	[["¿Tenés armas, Croft?"]],

	{"name", "Croft"},
	[["Eh usado armas improvisadas nada mas, pero si me he salvado es de
suerte."]],

	{"name", "Bake"},
	[["¿Y sabés disparar?"]],

	{"name", "Croft"},
	[["Eso si. Aprendí en el servicio," dijo así como si nada. Creo que
ninguno se lo esperó eso. No parecía militar.]],

	{"name", "Falcon"},
	[["¿De veras? ¿Y aún así no te dejaron salir?"]],

	{"name", "Croft"},
	[["Eh, pasó que no lo terminé porque accidentalmente le prendí fuego a
una bodega con municiones. Así que sigo siendo civil." Croft hablaba con
tanta sencillez que no sabía si era en serio o si nos jugaba una broma.]],

	{"name", "Bake"},
	[["lol, bueno, yo y Falcon tenemos armas, pero estaba pensando que
esto va para rato así que deberiamos buscar más, aparte de guitarras.
¿Nos acompañás?"]],

	{"name", "María"},
	[["Guita-- ¿Quieres mas?"]],

	{"name", "Falcon"},
	[["Yo tambien quiero. Me gusta el bajo pero un hombre necesita
variedad."]],

	{"name", "Croft"},
	[["Yo necesito gasolina para mi camioneta, pero dale."]],

	{"name", "María"},
	[["¿Soy la única que piensa en la comida?"]],

	{"name", "Croft"},
	[["¿No habías salido por un cepillo y pasta de dientes?"]],

	{"name", "María"},
	[["Ah, verdad. Vamos entonces."]],

	[[Agarramos nuestros abrigos --excepto por Croft-- y salimos al patio.
Pipi nos olfeateó los pies de nuevo al salir.]],

	{"name", "Bake"},
	[["Che, ¿en cual carro vamos?"]],

	{"name", "Falcon"},
	[["¿Como que en cual vamos 'che'? En el mio boludo. ¿O querés
congelarte las pelotas en el pickup todo el camino?" dijo imitándole el acento.
Bake entrecerró los ojos.]],

	{"name", "Bake"},
	[["Te dí comida falopero de mierda." Croft y yo los mirabamos tensos.]],

	{"name", "María"},
	[["Chicos..." les dije interponiéndome entre ellos antes que se
agarraran a tunazos. Los dos entonces se pusieron a reír, se dieron palmadas en
la espalda que retumbaron en toda la calle, y se subieron al auto de Falcon.]],

	[[Nos miramos yo y Croft con cara de wtf, y sin comentar nada nos
subimos atrás. Rocco saltó tras nosotros.]],

	{"name", "Croft"},
	[["Rocco, te llevo solo si te comportas."]],

	{"name", "Rocco"},
	[["¡Auf!"]],

	{"name", "Falcon"},
	[["¿Eh? ¿Por que?" preguntó Falcon mientras buscaba los
cables.]],

	{"name", "Croft"},
	[["Nada, nada..."]],
}
