return {
	{"style", "vnMaria"},
	{"name", false},
	[["María, María." Alguien me estaba despertando.]],

	res.fun.macro.title,

	{"name", "María"},
	[[Abrí aturdida los ojos, pero todo estaba oscuro. '¿Me golpée la
cabeza de nuevo?' fue lo primero que pensé, por alguna razón. Luego distinguí
una mano acercandose y la agarré pensando que me iba a tocar el chichón.
La persona exclamó asustada. Yo me senté de golpe.]],

	[[Con los ojos bien abiertos me dí cuenta que era Falcon.]],

	{"name", "Falcon"},
	[["¿Estas bien?" dijo sobresaltado. Miré a todos lados. Me encontraba
en el cuarto que ahora era de Bake, y mi cabeza... me dolía, pero no mas de lo
normal. "¿María?"]],

	{"name", "María"},
	[["Disculpa, pensé que... nada." Me refregué los ojos y miré la
ventana, dandome cuenta que recién estaba aclarando afuera. "¿Que pasó?
¿Que hora es?"]],

	{"name", "Falcon"},
	[["Croft se fue. ¿Sabes donde está tu revolver?"]],

	{"name", "María"},
	[["¿Ah?" dije totalmente perdida. "¿Que Croft que?"]],

	{"name", "Falcon"},
	[["¿Lo sabes?" insistió. De alguna forma logré hacer
memoria a esa hora indecente.]],

	{"name", "María"},
	[["Está... en mi abrigo," dije cerrando los ojos. "Con el que salí
ayer. Lo dejé en la silla anoche."]],

	{"name", "Falcon"},
	[["Demonios," murmuró. "¿Estás segura?"]],

	{"name", "María"},
	[["Si. ¿Que pasó? ¿Que es eso de que Croft se fue?"]],

	{"name", "Falcon"},
	[["Se fue con su camioneta y su perro. Y tu revolver al parecer."]],

	{"name", "María"},
	[["¿A-Ahora?"]],

	{"name", "Falcon"},
	[["No sé, subí al baño y ví su cuarto abierto. Se me ocurrió
que nos había robado algo, pero estaban las armas y la comida... Hasta
que revisé tu abrigo."]],

	{"name", "María"},
	[["¿N-No habrá salido a buscar algo?"]],

	{"name", "Falcon"},
	[["¿Como que?" respondió simplemente. Si, era una tontera lo que decía,
¿pero como iba a dormir y luego irse solo con mi arma? Le encontraba aún
menos sentido.]],

	[["Igual bajamos mucho la guardia. Deberiamos tener mas cuidado con
quien dejamos entrar. Nos podría haber matado."]],

	{"name", "María"},
	[["No digas eso."]],

	{"name", "Falcon"},
	[["Yo solo digo," dijo sombrio. No podía negar que tenía razón... es
solo que tampoco podía pensar que Croft pudiera hacer eso. O bueno, nadie mas
bien dicho.]],

	{"name", "María"},
	[["¿Y Bake donde está?" A Falcon se le formó una pequeña sonrisa apenas
lo mencioné.]],

	{"name", "Falcon"},
	[["Está abajo durmiendo en la alfombra. No lo pude despertar."]],

	{"name", "María"},
	[["¿Durmieron en el suelo ustedes? ¿Que están locos?"]],

	{"name", "Falcon"},
	[["No, solo el xD Yo dormí en el sillón xD"]],

	{"name", "María"},
	{"case", "DespiertaDonde", {
		sola = {
			[["Oh... Lo siento por tu espalda." No tanto.]],

			{"name", "Falcon"},
			[["Si, gracías xd"]],
		},
	}, {
		[["¿Y como dormiste?"]],

		{"name", "Falcon"},
		[["Terrible. No sé para que tiene cojines si uno toca las
		tablas igual xd Me desperté como seis veces xdd"]],
	}},

	[["Mas encima Bake me insitió que durmiera ahí, así que el
maldito debía saber como era. Ya me las va a pagar."]],

	{"name", "María"},
	[[A-ja. En lo que a mi respectaba, mucha simpatía no les tenía
considerando el ruido de anoche. Falcon se puso de pie.]],

	{"name", "Falcon"},
	[["Me voy a preparar el desayuno, por si quieres bajar. Voy a hacer el
pollo y el puré que es lo único que hay."]],

	{"name", "María"},
	[["Lo pensaré," dije mientras me recostaba de nuevo. Era muy temprano.]],

	{"name", "Falcon"},
	[[Falcon levantó los hombros y se fue hacia la puerta, por lo visto ya
casi curado de su pie. Iba rodeando la cama cuando pareció perder el equilibrio
por un momento, pero se afirmó en la tabla.]],

	{"name", "María"},
	[["¿Que te pasó? ¿Es tu pie?"]],

	{"name", "Falcon"},
	[["No, no, solo un mareo. Ya casi sospecho que me voy a quedar así."]],

	{"name", "María"},
	[["No digas eso."]],

	{"name", "Falcon"},
	[["No es tanto como ayer al menos." Falcon se fue con cuidado
hasta la puerta. Yo me volví a acostar.]],

	{"name", "María"},
	[[¿Que habrá pasado con Croft? Guardaba algo de esperanza en que
volvería. ¿Para que se iba a robar un revolver si ya tenía un arma?]],

	[[Me cubrí con las frazadas para meditar sobre aquello, en sueños.
No sentía hambre aún... Hasta que el olor a comida me empezó a llamar.
Que remedio. Me vestí y bajé al primer piso.]],

	[[En la sala me encontré a Bake cubierto con dos toallas en la
alfombra.]],

	[[Dije que no tenía simpatía, pero al verlo ahí durmiendo en el suelo
me conmoví un poco. ¿Por que no me había despertado? Ahí se iba a enfermar.
Me acerqué a moverlo.]],

	[["Bake, Bake." Como no reaccionó lo empecé a agitar. Bake se volvió
hacia el otro lado, pero yo insistí. Tras como un cuarto de hora reaccionó.
Para entonces se me había pasado la pena.]],

	{"name", "Bake"},
	[["¿Que pasa?" Sonreía somnoliento.]],

	{"name", "María"},
	[["Bake te vas a enfermar ahí."]],

	{"name", "Bake"},
	[["Naaaah..." Se giró hacia el sillón.]],

	{"name", "María"},
	[["¿No tienes frio?" Le toqué la cara y de alguna forma estaba
calentito.]],

	{"name", "Bake"},
	[["Not even in the cold november rain... (8)"]],

	{"name", "María"},
	[["Oye, Croft se fue."]],

	{"name", "Bake"},
	[["¿Ahh?"]],

	{"name", "María"},
	[["Que Croft se fue."]],

	{"name", "Bake"},
	[["Hmm, si el quiere estar solo, bueno xD..."]],

	{"name", "María"},
	[["¿No te importa?"]],

	{"name", "Bake"},
	[["Creo que no vendrá con nosotros hasta que esté por morir xD"]],

	{"name", "María"},
	[["Pero se llevó mi revolver además de su arma."]],

	{"name", "Bake"},
	[["Hmm, seguro se lo llevó sin balas, no es para tanto xD" Bake tenía
respuesta para todo.]],

	{"name", "María"},
	[[Me fui al comedor a revisar mi abrigo, y efectivamente, mi revolver
no estaba pero si el cargador, tal como había dicho. Era todo tan extraño.]],
}
