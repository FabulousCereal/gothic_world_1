return {
	{"style", "vnMaria"},
	res.fun.macro.title,

	{"name", "María"},
	[[La tormenta había comenzado a golpear fuerte cuando llegamos al
auto, con la nieve cayendo a montones. Falcon se apresuró en subirse al asiento
del conductor antes que Bake, y partimos con toda prisa hacia la casa.]],

	[["Bake, ¿va a durar mucho esta tormenta?"]],

	{"name", "Bake"},
	[["Ni idea. Esta es como la vuelta del invierno en primavera,
pero desde que era chico que no ocurría. No sé que irá a pasar," respondió.
Primavera... Recordé que era noviembre, que era primavera ahora y que esta
misma semana hicieron 30 grados donde vivía.]],

	[[No va a haber día en que no me arrepienta de haber venido a esta
puta ciudad.]],

	{"name", "Falcon"},
	[["Puede que den el prónostico en la radio."]],

	{"name", "Bake"},
	[["Bien pensado." Bake prendió la radio, y se puso a buscar la
estación que habiamos encontrado.]],

	{"name", "Fabian"},
	[["¿Descubrieron la estación de noticias?" preguntó Fab, inclinándose
entre los dos asientos.]],

	{"name", "María"},
	[["¿La conocian?"]],

	{"name", "Fabian"},
	[["Si. Teníamos una radio portátil donde la descubrimos. Nos alegramos
bastante, hasta que nos dimos cuenta de que era una grabación que se repite."]],

	{"name", "María"},
	[[Bake dejó de girar la perilla con esas palabras, y nos quedamos en
silencio con la estática mientras lo asimilabamos.]],

	{"name", "Falcon"},
	[["¿Una grabación?" preguntó Falcon incrédulo.]],

	{"name", "Fabian"},
	[["Si. Dura mas o menos una hora. Cuando termina hay un silencio, y
luego parte de nuevo," nos explicó. "No creo que sea algo bueno."]],

	{"name", "María"},
	[["O-O sea que eso de los 12 paises..."]],

	{"name", "Fabian"},
	[["Lo oímos hace dos días, pero puede ser de antes. Hasta del día
siguiente a la explosión, incluso" dijo. O sea que mi papá, mi hermano, los
papás de Bake...]],

	{"name", "Falcon"},
	[["Mierda. ¿Como se pudo expandir tan rapido?" dijo
Falcon sobándose la sien.]],

	{"name", "Bake"},
	[["La nube era gigante..." murmuró Bake.]],

	{"name", "Falcon"},
	[["¿Pero tan grande?"]],

	{"name", "Fabian"},
	[["Pasó la otra vez con un incendio grande en Australia, donde el humo
llegó a sudamérica. Puede ser lo mismo. También podría llegar a África,
supongo yo."]],

	{"name", "María"},
	[["Dios..."]],

	{"name", "Falcon"},
	[["Pero podrían ser solo brotes pequeños, ¿verdad? Si son solo un par
de personas, y los alcanzan a controlar, no sucedería lo de acá."]],

	{"name", "Fabian"},
	[["O sea, claro, no podemos asumir que se haya salido de control como
acá..." razonó Fab. "Pero estuve escuchando la onda corta, y todas las
estaciones de otros lados han ido desapareciendo. Puede que sea que la gente se
está resguardando... puede que no."]],

	{"name", "Falcon"},
	[["Espera. Onda corta me suena," dijo Falcon tratando de pensar.
"Ehh, es una frecuencia de muy largo alcance, ¿no es así?"]],

	{"name", "Fabian"},
	[["Asi es, abarca casi todo el continente. Rebota en lo
alto de la atmósfera así que la curvatura de la Tierra o la cordillera no la
bloquean. Solo necesitas encontrar una radio que la pueda recibir."]],

	{"name", "Bake"},
	[["¿Escucharon alguna de noticias nuevas?"]],

	{"name", "Fabian"},
	[["Aparte de la grabada, no. Eran todas de música, pero sin
locutores o gente hablando en vivo. La última que quedaba hasta anoche era
una de música en portugués, pero esa también ya desapareció." Yo y Bake
nos mirabamos. Esto no era bueno.]],

	{"name", "Falcon"},
	[["¿Y no traes esa radio?"]],

	{"name", "Fabian"},
	[["No, la abandonamos esta mañana. Era pesada y ninguno quería
cargarla."]],

	{"name", "Andrea"},
	[["Se le agotó la batería, y como no se oía nada, ¿para que?"
Ninguno de los dos chicos se veían preocupados, a diferencia de nosotros.
Quizás ellos ya habían aceptado la situación, pero para mi esto era terrible.
¿Que ibamos a hacer ahora?]],

	[[Miré a Bake, y el se pasaba la mano por el pelo.
Finalmente suspiró.]],

	{"name", "Bake"},
	[["Bue... Si salir de la ciudad con este clima hubiera sido arriesgado,
ahora no tiene ningún sentido."]],

	{"name", "María"},
	[["No," susurré para mi misma. Sentía que a cada rato todo se perdía.
¿Que ibamos a hacer, si tenemos que quedarnos acá? ¿Esperar que los zombies se
murieran solos? ¿Terminarían algún día? ¿Sobreviviriamos lo suficiente para
verlo?]],

	[[Pensé en lo que dijo Croft sobre los militares, y me convencí de lo
que dijeron los chicos; si fue verdad, ya no debían estar ahí.]],

	{"name", "Andrea"},
	[["Mira el viento," le dijo Andrea a Fab apuntando afuera. La nieve se
levantaba del suelo en lenguas. Se desprendía de los árboles y techos.
"Menos mal encontramos gente. ¿Que habría pasado si nos agarraba
la tormenta sin refugio?"]],

	{"name", "Fabian"},
	[["Hmm, nada, el frio nos habría adormecido y nos hubieramos congelado
tranquilamente."]],

	{"name", "Andrea"},
	[["Aaah," dijo asintiendo. Yo me sequé las lágrimas, y Andrea se dió
cuenta. "¿Que pasa?" me preguntó.]],

	{"name", "María"},
	[["Nada," dije girándome para ocultar mi cara. No sirvió de mucho; en
el vidrio se veía mi reflejo. Bake se volteó a verme, aunque no decía nada.]],

	[[Andrea se quitó el poncho. Radiaba calor al igual que Bake, debía ser
por el ejercicio de recién.]],

	{"name", "Falcon"},
	[["¿No tenían donde dormir?" preguntó Falcon cambiando de tema.]],

	{"name", "Fabian"},
	[["Pues, si y no."]],

	{"name", "Andrea"},
	[["Somos errantes," dijo como si fuera divertido.]],

	{"name", "Fabian"},
	[["No somos de acá la verdad, así que dormiamos en cualquier lugar
que nos protegiera del viento."]],

	{"name", "Falcon"},
	[["¿Vienen de otro país entonces?"]],

	{"name", "Fabian"},
	[["Yo si."]],

	{"name", "Andrea"},
	[["Yo no, aunque vivo casi en la frontera. Venía de paseo con mi curso,"
dijo. ¿Curso? Sus palabras sonaban tan a normalidad que me distrajeron de mi
pena.]],

	{"name", "Bake"},
	[["¿Cuantos años tenés?" preguntó Bake. ¿De verdad no habló de
nada con ellos mientras venían?]],

	{"name", "Andrea"},
	[["17." Uno menos que yo.]],

	{"name", "Bake"},
	[["¿Y vos Fab?"]],

	{"name", "Fabian"},
	[["24." Falcon inclinó la cabeza.]],

	{"name", "Falcon"},
	[["Ehh... ¿Y ustedes se conocían de antes?" dijo cauteloso.]],

	{"name", "Andrea"},
	[["No. Nos conocimos hace un par de días. Llegamos al mismo
supermercado a refugiarnos," Falcon asintió ante esas palabras.]],

	{"name", "Falcon"},
	[["Ahh, entonces, ¿no son novios?" Dios.]],

	{"name", "Andrea"},
	[["¿Que? No," dijo extrañada. "¿Por que dices eso?"]],

	{"name", "Falcon"},
	[["Ehh..."]],

	{"name", "Bake"},
	[["Disculpá que me meta," interrumpió Bake para salvarlo. "¿Pero que
pasó con tus curso?" La expresión de Andrea cambió un poco.]],

	{"name", "Andrea"},
	[["La verdad, no lo sé," respondió. "Nos trajeron el lunes a conocer la
ciudad, y teníamos que volver el miércoles, y como la explosión fue el martes
en la tarde, nos ordenaron quedarnos en el hotel en calma hasta que llegara el
bus al día siguiente."]],

	[["Yo compartía habitación con tres compañeras, pero no hablaba mucho
con ellas. Vimos las noticias en la noche, nos fuimos a dormir y todo, y como a
las seis de la mañana nos despertaron unos gritos desesperados en el piso
nuestro."]],

	[["Mientras que mis compañeras trataban de llamar a los demás por
teléfono, porque no se atrevían a salir del cuarto, yo decidí que no aguantaba
esto y junté mis cosas para irme a casa por mi cuenta."]],

	[["Ellas me dijeron que era peligroso, que iban a llamar al profesor
si salía, así que salí corriendo para huir antes que alguien me detuviera."]],

	[["Ehh, y bueno, cuento corto, logré llegar a un terminal de buses al
mediodía, pero estaba todo cerrado así que no sirvió de nada. Me quedé sin
volver a casa. Tampoco volví a ver a nadie."]],

	{"name", "Bake"},
	[["¿Y saliste sola, sin ningún amigo?"]],

	{"name", "Andrea"},
	[["No tenía amigos," respondió. Nos quedamos callados, y el ambiente se
puso algo incómodo.]],

	[[Era raro, aunque extrañamente conveniente que no haya perdido
a nadie cercano... Bueno, al igual como ocurre cuando tus padres están fuera o
porque no recuerdas a nadie, supongo.]],

	{"name", "Bake"},
	[["Y luego conociste a Fab."]],

	{"name", "Andrea"},
	[["A los dos días después, en realidad." ¿Dos días? Era bastante,
considerando que yo no duro ni 15 minutos afuera sin que me pase alguna
estupides.]],

	{"name", "María"},
	[["¿No viste mas gente antes de encontrar a Fab?"]],

	{"name", "Andrea"},
	[["Ví grupos saqueando almacenes, pero no me iba a acercar
a ellos, obviamente."]],

	{"name", "Bake"},
	[["¿Y tu Fab? ¿A que venías?"]],

	{"name", "Fabian"},
	[["Venía a quedarme unos días con una prima que no veo hacen años."]],

	[["Se suponía que me iba a buscar al aeropuerto, pero tuve la
suerte de que la fábrica explotara a los minutos de bajarme del avión.
Hubiera sido un poco antes, y me hubieran desviado a otro lado."]],

	[["En fin. Llegué como pude a un hostal, y al despertar ya no había
civilización. No había luz ni gente. Me conseguí un mapa y he caminado desde
entonces."]],

	{"name", "Bake"},
	[["¿Y tu prima?"]],

	{"name", "Fabian"},
	[["La ando buscando. No sé donde vive." Buscar a alguien
en una ciudad... Suena imposible.]],

	{"name", "Falcon"},
	[["¿Pero no fue al aeropuerto?"]],

	{"name", "Fabian"},
	[["No sé si habrá ido. Hubo un caos porque el humo se vino hacia el
aeropuerto muy rápido, y toda la gente empezó a salir, así que a lo mejor huyó,
o quizás se quedó dormida. No sé."]],

	{"name", "Bake"},
	[["¿Y no la llamaste?"]],

	{"name", "Fabian"},
	[["No tengo celular." Uf.]],

	{"name", "Bake"},
	[["lol, fail." Bake tenía razón. De los dos, cada uno era mas raro que
el otro.]],

	{"name", "Fabian"},
	[["Me gusta viajar ligero, ¿ok?" dijo apretando la mochila que
llevaba.]],

	{"name", "Falcon"},
	[["¿Como buscas a alguien que no sabes donde vive?" preguntó Falcon.
Imaginé que el tema le interesaba.]],

	{"name", "Fabian"},
	[["Ah, sucede que cuando venía de niño, ibamos a jugar a un parque
gigante que había cerca, así que me conseguí un mapa de la ciudad, anoté los
parques grandes, y me puse a caminar."]],

	{"name", "Falcon"},
	[["Suena tedioso."]],

	{"name", "Bake"},
	[["Acá el mas grande es el intercomunal Las Camelias."]],

	{"name", "Fabian"},
	[["Si, el que está cerca de la Avenida. De ahí venimos."]],

	{"name", "Bake"},
	[["Entonces no sé xd"]],

	{"name", "Fabian"},
	[["xd bueno, anoté 5 y ahora me quedan 3."]],

	{"name", "Falcon"},
	[["¿Pero no te acuerdas de alguna calle, o algo?"]],

	{"name", "Fabian"},
	[["Por mas que trato, nada xd"]],

	{"name", "Falcon"},
	[["Bueno, no hay nada peor que no poder recordar..." dijo
Falcon medio suspirando. Andrea habló.]],

	{"name", "Andrea"},
	[["Me imagino. Bake nos estuvo contando un poco..." dijo solemne.
O sea que algo hablaron. "¿En serio estabas aspirando lineas al lado de la
la fábrica cuando explotó?"]],

	{"name", "María"},
	[[Falcon mantuvo la vista fija mientras Bake y yo nos estallabamos
de risa. Los dos chicos miraban confundidos.]],

	{"name", "Falcon"},
	[["Yo no me drogo," se limitó a decir. Si no estuviera conduciendo,
le estaría dando tunasos a Bake en este instante.]],

	{"name", "Bake"},
	[["xD worth it"]],

	{"name", "Andrea"},
	[["A-Ah, ok. Igual que terrible perder la memoria," dijo tratando de
arreglarla. A mi se me escapó otra risa. "¿Pero de verdad no recuerdas nada
de nada?"]],

	{"name", "Falcon"},
	[["Reconozco lugares famosos como la Avenida, pero eso es
todo. Ni siquiera sé en que trabajaba."]],

	{"name", "Andrea"},
	[["¿Como Bake dijo que eras...? Nada." Bake reprimió otra risa.]],

	{"name", "Fabian"},
	[["E-Ejem," dijo Fab. "Pero lo de que estabas al lado de la fábrica,
¿es verdad?"]],

	{"name", "Falcon"},
	[["Si, eso si. Estaba con mi novia. Caí inconsciente con la explosión,
y cuando desperté ella ya no estaba. Tampoco tenía recuerdos."]],

	{"name", "Andrea"},
	[["Guau..."]],

	{"name", "Fabian"},
	[["Dijeron que fue una fábrica de tapas de inodoros la que explotó.
¿Es verdad eso también?"]],

	{"name", "Bake"},
	[["lol, ¿que?" Andrea apoyó la sien en la mano.]],

	{"name", "Andrea"},
	[["Fab siempre menciona eso cuando sale el tema."]],

	{"name", "María"},
	[["Yo también escuche eso. En las noticias, antes que se cortara la
luz."]],

	{"name", "Fabian"},
	[["¡Ah! ¿Ves? No estoy loco. ¿Eh? ¡¿Eh?!" dijo pinchando a Andrea con
el codo. Andrea empezó a pinchar de vuelta.]],

	{"name", "Falcon"},
	[["Bueno, cuando desperté el edificio ya era una pila de escombros, así
que no puedo asegurar que allí hayan hecho inodoros."]],

	{"name", "Bake"},
	[["Yo no escuché nada de eso, y es tan random que no sé si sería
lolero, quizás ni siquiera equisdé." wtf]],

	{"name", "Fabian"},
	[["Es que me pareció raro. ¿Que puede tener una fábrica de tapas de
inodoros para que convierta a la gente en zombies?" Lo mismo que me había
preguntado yo.]],

	{"name", "Andrea"},
	[["Los periodistas se pudieron haber confundido de lugar."]],

	{"name", "Bake"},
	[["Si, yo creo que fue eso, porque en el canal que ví dijeron que
estaban a la espera de poder confirmar."]],

	{"name", "Fabian"},
	[["¿Pero entonces que estalló?"]],

	{"name", "Andrea"},
	[["Podrías ir a ver."]],

	{"name", "Fabian"},
	[["De hecho pensaba hacerlo. Tras encontrar a mi prima, claro."]],

	{"name", "Bake"},
	[["lol, te va a matar la radiación si te acercás."]],

	{"name", "Fabian"},
	[["Pero Falcon sobrevivió sin problemas. ¿Verdad?" preguntó Fab, y nos
quedamos en silencio. "¿Verdad?" Creo que ninguno de los tres quiso arriesgarse
a que Falcon empezara de nuevo con que tenía cáncer. "¿Eh?"]],

	{"name", "Falcon"},
	[["Ahem, ya vamos llegando," dijo metiéndose a nuestra calle. La nieve
ya tapaba todo, pero aún se distinguía el portón abierto de la casa del vecino,
tal como lo dejamos. Bajé la cabeza al pasar por en frente. Iba a tener a
resignarme a la idea de dormir con dos cadáveres al lado.]],

	{"name", "Bake"},
	[["¿Que hace Pipi afuera?" preguntó Bake al ver su casa. Miramos y
claro, estaba Pipi en medio del patio, tiritando con el viento. Falcon se
estacionó. "¿No se habrá metido algo?"]],

	{"name", "Falcon"},
	[["No mames." Observamos atentos la casa por unos momentos. No se veía
ningún movimiento, tampoco ningún ruido, pero ante la duda Falcon y Bake
sacaron sus armas.]],
}
